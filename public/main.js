...
if (dashColorRow && dashSizeRow) {
    dashColorRow.onclick = function (e) {
    ...
    dashSizeRow.onclick = function (e) {
    ...
    if (customColorInput && colorPickerBtn) {
        customColorInput.onchange = function () {
            var colorVal = this.value;
            if (!colorVal) return;

            var existing = dashColorRow.querySelector('.color-dot[data-color="' + colorVal + '"]');
            if (existing) {
            ...
            newDot.setAttribute("data-color", colorVal);
            newDot.setAttribute("aria-label", colorVal);
            newDot.setAttribute("title", colorVal);
            newDot.style.backgroundColor = colorVal;

            dashColorRow.insertBefore(newDot, colorPickerBtn);
            collectSelected(dashColorRow, "available-colors", "data-color");

            this.value = "#" + Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0');
        };
    }
}
